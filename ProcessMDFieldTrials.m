...
receivedRecordings =   dir('D:\Data\CABOW ID');
...
clipTableDir =dir(fullfile(headDir, ...
    'Basestation  Database\CSV exports\ClassifierNoiseUpcall*'));
clipTable =readtable(fullfile(clipTableDir(end).folder, clipTableDir(end).name));
clipTable.UTC.TimeZone = 'UTC';

edgeTableDir =dir(fullfile(headDir, ...
    'Basestation  Database\CSV exports\RightWhale*'));
edgeTable = readtable(fullfile(edgeTableDir(end).folder, edgeTableDir(end).name));
...
%     % expected arrival times on the associated CABOW
...
writetable(SLtable, 'C:\BitBucketRepositories\CABOWProcessing\ProcessedCSVfiles\MDTrials.csv')